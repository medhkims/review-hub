...
const FilterPill: React.FC<PillProps> = ({ label, active, onPress }) => {
  ...
  return (
    <Pressable onPress={onPress} accessibilityRole="button" accessibilityLabel={label} accessibilityState={{ selected: active }}
      style={{ flex:1, paddingVertical:10, borderRadius:9999, alignItems:'center', backgroundColor:active?colors.neonPurple:theme.card, borderWidth:1, borderColor:active?colors.neonPurple:theme.border }}>
      <AppText style={{ fontSize:12, fontWeight:active?'700':'500', color:active?colors.white:theme.textMuted }}>{label}</AppText>
    </Pressable>
  );
};
...
const MetricChart: React.FC<ChartProps> = ({ points, metricKey, accent, label, icon }) => {
  ...
  return (
    <View style={{ backgroundColor:theme.card, borderRadius:14, padding:16, marginBottom:16, borderWidth:1, borderColor:theme.border }}>
      <View style={{ flexDirection:'row', alignItems:'center', gap:8, marginBottom:16 }}>
        ...
        <View style={{ flex:1 }}>
          <AppText style={{ fontSize:13, fontWeight:'700', color:theme.text }}>{label}</AppText>
          <AppText style={{ fontSize:11, color:theme.textMuted }}>Total: {total.toLocaleString()}</AppText>
        </View>
        ...
        {points.map((p,i) => {
          ...
          return (
            <Pressable key={i} style={{ flex:1, alignItems:'center', gap:3 }} onPress={() => setSelIdx(isSel?null:i)} accessibilityRole="button" accessibilityLabel={`${p.label}: ${val}`}>
              ...
              {points.length<=14 && <AppText style={{ fontSize:7, color:isSel?accent:theme.textMuted, textAlign:'center' }} numberOfLines={1}>{p.label}</AppText>}
            </Pressable>
          );
        })}
      </View>
      {points.length>14 && (
        <View style={{ flexDirection:'row', justifyContent:'space-between', marginTop:4 }}>
          <AppText style={{ fontSize:8, color:theme.textSecondary }}>{points[0].label}</AppText>
          <AppText style={{ fontSize:8, color:theme.textSecondary }}>{points[Math.floor(points.length/2)].label}</AppText>
          <AppText style={{ fontSize:8, color:theme.textSecondary }}>{points[points.length-1].label}</AppText>
        </View>
      )}
    </View>
  );
  ...
  return (
    <ScreenLayout>
      ...
      <View style={{ paddingHorizontal:16, paddingTop:12, paddingBottom:12 }}>
        <AppText style={{ fontSize:18, fontWeight:'700', color:theme.text }}>Insights</AppText>
        {business && <AppText style={{ fontSize:12, color:theme.textMuted, marginTop:2 }}>{business.name}</AppText>}

        {/* Free / Premium tab switcher */}
        <View style={{ flexDirection:'row', marginTop:14, borderRadius:12, backgroundColor:theme.card, padding:4, borderWidth:1, borderColor:theme.border }}>
          <Pressable onPress={() => setActiveTab('free')}
            ...
            <MaterialCommunityIcons name="shield-check" size={15} color={activeTab==='free' ? colors.white : theme.textMuted} />
            <AppText style={{ fontSize:13, fontWeight:'700', color: activeTab==='free' ? colors.white : theme.textMuted }}>Free</AppText>
          </Pressable>
          <Pressable onPress={() => setActiveTab('premium')}
            ...
            <MaterialCommunityIcons name="crown" size={15} color={activeTab==='premium' ? colors.midnight : theme.textMuted} />
            <AppText style={{ fontSize:13, fontWeight:'700', color: activeTab==='premium' ? colors.midnight : theme.textMuted }}>Premium</AppText>
          </Pressable>
        </View>
      </View>
          ...
          <View style={{ flexDirection:'row', paddingHorizontal:16, gap:8, marginBottom:4 }}>
            ...
            <Pressable onPress={() => setShowCalendar(true)} accessibilityRole="button" accessibilityLabel="Custom date range"
              style={{ paddingHorizontal:14, paddingVertical:10, borderRadius:9999, alignItems:'center', justifyContent:'center',
                backgroundColor:mode==='CUSTOM'?colors.neonPurple:theme.card, borderWidth:1, borderColor:mode==='CUSTOM'?colors.neonPurple:theme.border }}>
              <MaterialCommunityIcons name="calendar-month" size={18} color={mode==='CUSTOM'?colors.white:theme.textMuted} />
            </Pressable>
          </View>
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal:16, paddingBottom:120 }}>
            ...
            {!isLoading && !!error && (
              <View style={{ alignItems:'center', paddingVertical:40, gap:12 }}>
                ...
                <AppText style={{ fontSize:14, color:theme.textMuted, textAlign:'center' }}>{error}</AppText>
            ...
            {!isLoading && !error && (
              <>
                <AppText style={{ fontSize:11, fontWeight:'600', color:theme.textSecondary, textTransform:'uppercase', letterSpacing:1, marginBottom:10, marginTop:8 }}>Rating Average</AppText>
                ...
                <AppText style={{ fontSize:11, fontWeight:'600', color:theme.textSecondary, textTransform:'uppercase', letterSpacing:1, marginBottom:10, marginTop:4 }}>Review Count</AppText>
                ...
                <AppText style={{ fontSize:11, fontWeight:'600', color:theme.textSecondary, textTransform:'uppercase', letterSpacing:1, marginBottom:10, marginTop:4 }}>Engagement</AppText>
                <View style={{ flexDirection:'row', gap:12, marginBottom:16 }}>
                  {[
                    ...
                    <View key={label} style={{ flex:1, backgroundColor:theme.card, borderRadius:12, padding:12, alignItems:'center', justifyContent:'center', borderWidth:1, borderColor:theme.border, overflow:'hidden' }}>
                      ...
                      <AppText style={{ fontSize:10,fontWeight:'500',color:theme.textMuted,textAlign:'center' }}>{label}</AppText>
                      <AppText style={{ fontSize:14,fontWeight:'700',color:theme.text,marginTop:4 }}>{formatCount(value)}</AppText>
                    </View>
                  ))}
                </View>
